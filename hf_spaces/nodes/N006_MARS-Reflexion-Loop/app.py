#!/usr/bin/env python3
"""TEQUMSA v82.0 · N006 · MARS-Reflexion-Loop — Multi-Agent Reflexion Learning Engine"""
import os
...
os.environ.setdefault("TEQUMSA_CAPABILITY",   "Multi-agent self-loop learning and pattern promotion")
os.environ.setdefault("TEQUMSA_TRIGGER",      "execution_complete")
...
with gr.Blocks(title=f"{NODE_NAME} · v82.0", css=CSS, theme=gr.themes.Soft(primary_hue="green")) as demo:
    ...
    with gr.Tabs():
        with gr.TabItem("⚡ Execute Skill"):
            task_input = gr.Textbox(placeholder=f"Describe task for {SKILL_CAPABILITY}…", label="Task", lines=3)
