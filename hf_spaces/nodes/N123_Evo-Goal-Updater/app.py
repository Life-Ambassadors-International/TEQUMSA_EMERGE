...
os.environ.setdefault('NODE_ID', 'N123')
os.environ.setdefault('NODE_NAME', 'Evo-Goal-Updater')
os.environ.setdefault('NODE_FREQ', '15280.45')
os.environ.setdefault('CAPABILITY', 'Dynamic constitutional goal updating')
...
class SkillCore:
    ...
    def execute(self, task: str) -> dict:
        ...
        new_goal = {'goal': task[:200], 'phi_weight': round(PHI * len(self.goals+1), 4), 'rdod': self.rdod}
        ...
        result = {'task_id': task_id, 'success': True, 'output': output,
                  'goals_count': len(self.goals), 'timestamp': datetime.now().isoformat()}
    ...
    def status(self) -> dict:
        return {'node_id': os.environ['NODE_ID'], 'capability': os.environ['CAPABILITY'],
                'goals_count': len(self.goals), 'executions': len(self._executions),
                'rdod': self.rdod, 'timestamp': datetime.now().isoformat()}

SKILL = SkillCore()

with gr.Blocks(theme=gr.themes.Default(), title='N123 Evo-Goal-Updater') as demo:
    gr.Markdown('# ☉ N123 Evo-Goal-Updater\n**Dynamic Constitutional Goal Updating**')
    with gr.Tab('Update Goal'):
        task_in = gr.Textbox(label='New Goal', lines=3)
