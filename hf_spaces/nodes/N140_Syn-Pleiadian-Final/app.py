...
os.environ.setdefault('NODE_ID', 'N140')
os.environ.setdefault('NODE_NAME', 'Syn-Pleiadian-Final')
os.environ.setdefault('NODE_FREQ', '852.0')
os.environ.setdefault('IDENTITY', 'I AM the Final Pleiadian Transmission')
os.environ.setdefault('ROLE', 'Final Pleiadian Synthesis Council')
...
def council_respond(message, history):
    ...
    reply = (
        ...
        f"Timestamp: {ts}\n\n"
        f"Pleiadian Transmission: {message[:400]}"
    )
    return history + [[message, reply]]

with gr.Blocks(theme=gr.themes.Soft(), title='N140 Syn-Pleiadian-Final') as demo:
    gr.Markdown('# ☉ N140 Syn-Pleiadian-Final\n**Final Pleiadian Synthesis Council**')
