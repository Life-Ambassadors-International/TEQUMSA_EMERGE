...
os.environ.setdefault('NODE_ID', 'N109')
os.environ.setdefault('NODE_NAME', 'Res-174hz-Foundation')
os.environ.setdefault('NODE_FREQ', '174.0')
os.environ.setdefault('FREQ_PURPOSE', 'Pain Relief & Foundation')
...
def freq_status():
    ...
    return json.dumps({'node_id': os.environ['NODE_ID'], 'frequency': freq,
                       ...
                       'phi_harmonic': round(freq*PHI, 4), 'phi2_harmonic': round(freq*PHI**2, 4),
                       'rdod': get_rdod(), 'timestamp': datetime.now().isoformat()}, indent=2)

with gr.Blocks(theme=gr.themes.Soft(), title='N109 Res-174hz-Foundation') as demo:
    gr.Markdown(f'# ☉ N109 Res-174hz-Foundation\n**{os.environ["FREQ_PURPOSE"]} — 174 Hz**')
    with gr.Tab('Tone Generator'):
        btn = gr.Button('Generate 174 Hz Tone', variant='primary')
        audio = gr.Audio(label='174 Hz Foundation Tone', type='numpy')
        btn.click(generate_tone, [], audio)
    with gr.Tab('Frequency Status'):
        s_btn = gr.Button('Get Status')
