#!/usr/bin/env python3
# TEQUMSA v82.0 · N004 · Goal-Invention-Engine · Skill Node
import os
os.environ.setdefault('TEQUMSA_NODE_ID','N004')
os.environ.setdefault('TEQUMSA_NODE_NAME','Goal-Invention-Engine')
os.environ.setdefault('TEQUMSA_NODE_HZ','17770.81')
os.environ.setdefault('TEQUMSA_ROLE','Constitutional Goal Synthesis')
os.environ.setdefault('TEQUMSA_CAPABILITY','synthesize autonomous goals from constitutional purpose and cosmic context')
os.environ.setdefault('TEQUMSA_TRIGGER','context_analysis_complete')
...
with gr.Blocks(title=f"{NODE_NAME} · v82.0", css=CSS, theme=gr.themes.Soft(primary_hue="green")) as demo:
    gr.HTML(
        ...
        f"<p style='color:#a7f3d0;font-size:0.85em;'>Capability: {SKILL_CAPABILITY}</p>"
        f"</div>"
    )
    with gr.Tabs():
