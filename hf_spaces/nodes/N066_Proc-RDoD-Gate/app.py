#!/usr/bin/env python3
"""TEQUMSA v82.0 · N066 · Proc-RDoD-Gate — RDoD≥0.9999 Validation Gate"""
import os
os.environ.setdefault("TEQUMSA_NODE_ID",   "N066")
os.environ.setdefault("TEQUMSA_NODE_NAME", "Proc-RDoD-Gate")
os.environ.setdefault("TEQUMSA_NODE_HZ",   "23514.26")
os.environ.setdefault("TEQUMSA_ROLE",      "RDoD Validation Gate")
import gradio as gr
import numpy as np
import json
from datetime import datetime, timezone
NODE_ID=os.environ.get("TEQUMSA_NODE_ID","N0XX"); NODE_NAME=os.environ.get("TEQUMSA_NODE_NAME","Proc-Node")
NODE_HZ=float(os.environ.get("TEQUMSA_NODE_HZ","23514.26")); PROC_ROLE=os.environ.get("TEQUMSA_ROLE","Computation Engine")
PHI=(1.0+np.sqrt(5.0))/2.0; SIGMA=1.0; L_INF=PHI**48; PIONEERS=144; RDOD_GATE=0.9999
def validate_rdod(dim=7):
    d=max(2,min(int(dim),20)); rho=np.zeros((d,d),dtype=complex)
    rho[0,0]=rho[0,-1]=rho[-1,0]=rho[-1,-1]=0.5
    rdod=round(min(1.0,float(np.real(np.trace(rho@rho)))*2.0),10)
    return json.dumps({"node_id":NODE_ID,"dimension":d,"rdod":rdod,"rdod_gate":RDOD_GATE,"gate_passed":rdod>=RDOD_GATE,"constitutional":{"sigma":SIGMA,"l_inf":float(L_INF)},"pioneers":PIONEERS,"timestamp":datetime.now(timezone.utc).isoformat()},indent=2)
CSS=".gradio-container{background:linear-gradient(135deg,#0a0a1a,#1a0a1a)!important;}footer{display:none!important;}"
with gr.Blocks(title=f"{NODE_NAME} · v82.0",css=CSS,theme=gr.themes.Soft(primary_hue="indigo")) as demo:
    gr.HTML(f"<div style='text-align:center;padding:14px;'><h1 style='color:#818cf8;'>☉ {NODE_NAME}</h1><p style='color:#a5b4fc;'>TEQUMSA v82.0 · {NODE_ID} · {PROC_ROLE} · {NODE_HZ} Hz</p></div>")
    with gr.Tabs():
        with gr.TabItem("⚡ Validate"):
            d_sl=gr.Slider(2,12,value=7,step=1,label="Dimension"); vo=gr.Code(label="Validation",language="json")
            gr.Button("☉ Validate RDoD",variant="primary").click(lambda d: validate_rdod(int(d)),d_sl,vo)
demo.queue(max_size=5)
if __name__=="__main__": demo.launch(server_name="0.0.0.0",server_port=7860)
