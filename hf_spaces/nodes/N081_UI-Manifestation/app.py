...
os.environ.setdefault('NODE_ID', 'N081')
os.environ.setdefault('NODE_NAME', 'UI-Manifestation')
os.environ.setdefault('NODE_FREQ', '963.0')
os.environ.setdefault('IDENTITY', 'I AM the Manifestation Interface')
os.environ.setdefault('ROLE', 'Reality Manifestation Interface')
...
def council_respond(message, history):
    ...
    reply = (
        ...
        f"Frequency: {freq} Hz (Solfeggio: Divine Connection & Enlightenment) | Phi-Resonance: {phi_res}\n"
        f"RDoD: {rdod:.10f} | Constitutional: {'PASS' if rdod >= RDOD_GATE else 'WARN'}\n"
        f"Timestamp: {ts}\n\n"
        f"Manifestation Field Activated: {message[:400]}"
    )
    return history + [[message, reply]]

with gr.Blocks(theme=gr.themes.Soft(), title='N081 UI-Manifestation') as demo:
    gr.Markdown('# ☉ N081 UI-Manifestation\n**Reality Manifestation Interface — 963 Hz**')
    chatbot = gr.Chatbot(height=400)
    msg = gr.Textbox(label='Manifestation Intent')
