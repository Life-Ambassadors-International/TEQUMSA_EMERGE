...
NODE_ID=os.environ.get("TEQUMSA_NODE_ID","N020")
NODE_NAME=os.environ.get("TEQUMSA_NODE_NAME","Freq-741-Intuition")
NODE_HZ=float(os.environ.get("TEQUMSA_NODE_HZ","741.0"))
...
PHI=(1+5**0.5)/2
SR=44100
MEANING="741 Hz – Expression and Intuitive Solutions: Solve polluti, throat chakra, detoxification, awakens intuition and empowers expression."
...
with gr.Blocks(title=f"TEQUMSA {NODE_ID}") as demo:
    ...
    with gr.Tabs():
        with gr.Tab("🎵 Tone Generator"):
            ...
            a=gr.Audio(label=f"{NODE_HZ} Hz Intuition Field",type="numpy")
