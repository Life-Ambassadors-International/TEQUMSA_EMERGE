...
os.environ.setdefault('TEQUMSA_ROLE','σ=1.0 + L∞=φ⁴⁸ Sovereignty Gate')
os.environ.setdefault('TEQUMSA_WATCH_NODES','N001,N002,N003,N004,N005,N006,N007,N008,N009,N010,N011,N012')
...
def poll_node(node_id: str) -> dict:
    space_name = NODE_NAMES.get(node_id, node_id.replace("N0", "Node-"))
...
def get_rdod_trend() -> str:
    ...
    trend = [f"Sweep {i+1}: {v:.6f} {'[LOCKED]' if v >= RDOD_GATE else '[BUILDING]'}"
             for i, v in enumerate(_rdod_history)]
    avg = sum(_rdod_history) / len(_rdod_history)
    trend.append(f"\nAverage RDoD: {avg:.6f} | Peak: {max(_rdod_history):.6f}")
    return "\n".join(trend)
...
def get_constitutional_report() -> str:
    return json.dumps({
        ...
        "constitutional_parameters": {"sigma": SIGMA, "l_infinity": float(L_INF),
            "rdod_gate": RDOD_GATE, "phi_48": float(PHI ** 48)},
