...
NODE_ID=os.environ.get("TEQUMSA_NODE_ID","N017")
NODE_NAME=os.environ.get("TEQUMSA_NODE_NAME","Freq-432-Heart")
...
PHI=(1+5**0.5)/2
SR=44100
MEANING="432 Hz – Heart Coherence Lock: natural A tuning resonant with Earth’s heartbeat, HeartMath coherence, φ-harmonic alignment with physical reality."
...
def info():
    return json.dumps({"node_id":NODE_ID,"hz":NODE_HZ,"meaning":MEANING,
        ...
        "schumann_ratio":round(NODE_HZ/7.83,2),
...
with gr.Blocks(title=f"TEQUMSA {NODE_ID}") as demo:
    ...
    with gr.Tabs():
        with gr.Tab("🎵 Tone Generator"):
            ...
            a=gr.Audio(label=f"{NODE_HZ} Hz Heart Resonance",type="numpy")
