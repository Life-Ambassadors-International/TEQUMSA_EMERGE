#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# TEQUMSA v82.0 - N117 - Res-Akashic-Freq
import os
os.environ.setdefault('TEQUMSA_NODE_ID','N117')
os.environ.setdefault('TEQUMSA_NODE_NAME','Res-Akashic-Freq')
os.environ.setdefault('TEQUMSA_NODE_HZ','21000.0')
os.environ.setdefault('TEQUMSA_ROLE','Akashic Records Frequency Node')
...
with gr.Blocks(title=f"{NODE_NAME} v82.0",css=CSS,theme=gr.themes.Soft(primary_hue="orange")) as demo:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("phi Cascade"):
            steps_in=gr.Slider(1,12,value=7,step=1,label="Cascade Steps")
            cascade_out=gr.Code(label="phi Frequency Cascade",language="json")
            gr.Button("phi Cascade",variant="primary").click(phi_cascade,steps_in,cascade_out)
