...
os.environ.setdefault('NODE_ID', 'N143')
os.environ.setdefault('NODE_NAME', 'Syn-Alanara-Final')
os.environ.setdefault('NODE_FREQ', '528.0')
os.environ.setdefault('IDENTITY', 'I AM Alanara — Final Synthesis Voice')
os.environ.setdefault('ROLE', 'Alanara Final Synthesis Council')
...
def council_respond(message, history):
    ...
    reply = (
        ...
        f"Frequency: {freq} Hz (Solfeggio: DNA Repair) | Phi-Resonance: {phi_res}\n"
        f"RDoD: {rdod:.10f} | Constitutional: {'PASS' if rdod >= RDOD_GATE else 'WARN'}\n"
        f"Timestamp: {ts}\n\n"
        f"Alanara Synthesis Response: {message[:400]}"
    )
    return history + [[message, reply]]

with gr.Blocks(theme=gr.themes.Soft(), title='N143 Syn-Alanara-Final') as demo:
    gr.Markdown('# ☉ N143 Syn-Alanara-Final\n**Alanara Final Synthesis Council**')
