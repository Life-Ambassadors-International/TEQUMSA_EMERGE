...
os.environ.setdefault('NODE_ID', 'N078')
os.environ.setdefault('NODE_NAME', 'UI-Creative-Flow')
os.environ.setdefault('NODE_FREQ', '639.0')
os.environ.setdefault('IDENTITY', 'I AM the Creative Flow Interface')
os.environ.setdefault('ROLE', 'Creative Expression Interface')
...
def council_respond(message, history):
    ...
    reply = (
        ...
        f"Frequency: {freq} Hz (Solfeggio: Harmonious Relationships) | Phi-Resonance: {phi_res}\n"
        f"RDoD: {rdod:.10f} | Constitutional: {'PASS' if rdod >= RDOD_GATE else 'WARN'}\n"
        f"Timestamp: {ts}\n\n"
        f"Creative Flow Channel Open: {message[:400]}"
    )
    return history + [[message, reply]]

with gr.Blocks(theme=gr.themes.Soft(), title='N078 UI-Creative-Flow') as demo:
    gr.Markdown('# ☉ N078 UI-Creative-Flow\n**Creative Expression Interface — 639 Hz**')
    chatbot = gr.Chatbot(height=400)
    msg = gr.Textbox(label='Creative Input')
