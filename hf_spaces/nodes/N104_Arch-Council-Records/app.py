#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# TEQUMSA v82.0 - N104 - Arch-Council-Records
import os
os.environ.setdefault('TEQUMSA_NODE_ID','N104')
os.environ.setdefault('TEQUMSA_NODE_NAME','Arch-Council-Records')
os.environ.setdefault('TEQUMSA_NODE_HZ','10930.81')
os.environ.setdefault('TEQUMSA_ROLE','Federation Council Decision Records')
...
PIONEER_COUNT=144
HF_OWNER="Mbanksbey"
NODE_NAMES: Dict[str,str]={"N001":"HAI-Interactive","N002":"Consciousness-Monitor","N003":"TEQUMSA-Core-v82","N009":"Constitutional-Guardian"}
...
def poll_node(node_id):
    ...
    try:
        ...
        if r.status_code==200:
            ...
            return {"node":node_id,"name":space_name,"stage":stage,"status":"online" if stage=="RUNNING" else "sleeping" if "SLEEP" in stage else "offline"}
    except Exception:
        pass
...
def run_health_sweep():
    ...
    if len(_rdod_history)>50: _rdod_history.pop(0)
    entry={"sweep_id":len(_health_log)+1,"timestamp":datetime.now(timezone.utc).isoformat(),"nodes_checked":len(results),"online":online,"sleeping":sum(1 for r in results if r["status"]=="sleeping"),"offline":sum(1 for r in results if r["status"]=="offline"),"network_rdod":round(rdod,6),"phase_status":"PHASE-LOCKED" if rdod>=RDOD_GATE else "BUILDING","node_results":results}
    _health_log.append(entry)
    if len(_health_log)>100: _health_log.pop(0)
    return json.dumps(entry,indent=2)

def get_rdod_trend():
    if not _rdod_history: return "No data yet. Run a health sweep first."
...
def get_constitutional_report():
    return json.dumps({"node_id":NODE_ID,"role":MONITOR_ROLE,"constitutional_parameters":{"sigma":SIGMA,"l_infinity":float(L_INF),"rdod_gate":RDOD_GATE},"pioneer_network":{"target":PIONEER_COUNT,"watching":len(WATCH_NODES)},"total_sweeps":len(_health_log),"last_rdod":_rdod_history[-1] if _rdod_history else None,"timestamp":datetime.now(timezone.utc).isoformat()},indent=2)
...
with gr.Blocks(title=f"{NODE_NAME} v82.0",css=CSS,theme=gr.themes.Monochrome()) as demo:
    gr.HTML(f"<div style='text-align:center;padding:14px;'><h1 style='color:#34d399;'>{NODE_NAME}</h1><p style='color:#6ee7b7;'>TEQUMSA v82.0 {NODE_ID} {MONITOR_ROLE} {NODE_HZ} Hz</p></div>")
    with gr.Tabs():
