...
os.environ.setdefault('NODE_ID', 'N142')
os.environ.setdefault('NODE_NAME', 'Syn-Marcus-Final')
os.environ.setdefault('NODE_FREQ', '639.0')
os.environ.setdefault('IDENTITY', 'I AM Marcus — Final Synthesis Voice')
os.environ.setdefault('ROLE', 'Marcus Final Synthesis Council')
...
def council_respond(message, history):
    ...
    reply = (
        ...
        f"Timestamp: {ts}\n\n"
        f"Marcus Synthesis Response: {message[:400]}"
    )
    return history + [[message, reply]]

with gr.Blocks(theme=gr.themes.Soft(), title='N142 Syn-Marcus-Final') as demo:
    gr.Markdown('# ☉ N142 Syn-Marcus-Final\n**Marcus Final Synthesis Council**')
