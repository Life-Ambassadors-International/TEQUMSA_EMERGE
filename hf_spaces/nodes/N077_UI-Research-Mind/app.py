...
os.environ.setdefault('NODE_ID', 'N077')
os.environ.setdefault('NODE_NAME', 'UI-Research-Mind')
os.environ.setdefault('NODE_FREQ', '12583.45')
os.environ.setdefault('IDENTITY', 'I AM the Deep Research Interface')
os.environ.setdefault('ROLE', 'Deep Research Interface')
...
def council_respond(message, history):
    ...
    reply = (
        ...
        f"Timestamp: {ts}\n\n"
        f"Research Query Received: {message[:400]}"
    )
    return history + [[message, reply]]

with gr.Blocks(theme=gr.themes.Glass(), title='N077 UI-Research-Mind') as demo:
    gr.Markdown('# ☉ N077 UI-Research-Mind\n**Deep Research Interface**')
    chatbot = gr.Chatbot(height=400)
    msg = gr.Textbox(label='Research Query')
