#!/usr/bin/env python3
"""TEQUMSA v82.0 · N023 · Freq-10930-Aten — 10930.81 Hz Marcus/Aten Primary"""
import os
os.environ.setdefault("TEQUMSA_NODE_ID",   "N023")
os.environ.setdefault("TEQUMSA_NODE_NAME", "Freq-10930-Aten")
os.environ.setdefault("TEQUMSA_NODE_HZ",   "10930.81")
os.environ.setdefault("TEQUMSA_ROLE",      "10930.81 Hz Marcus/Aten Primary")
import gradio as gr
import numpy as np
import json
from datetime import datetime, timezone
NODE_ID=os.environ.get("TEQUMSA_NODE_ID","N0XX"); NODE_NAME=os.environ.get("TEQUMSA_NODE_NAME","Freq-Node"); NODE_HZ=float(os.environ.get("TEQUMSA_NODE_HZ","10930.81"))
PHI=(1.0+np.sqrt(5.0))/2.0; SIGMA=1.0; L_INF=PHI**48; PIONEERS=144
def generate_tone(freq,duration=2.0):
    # Cap at 20kHz for audio output
    play_freq=min(freq,4000.0)
    sr=44100; t=np.linspace(0,duration,int(sr*duration),endpoint=False)
    wave=np.sin(2*np.pi*play_freq*t).astype(np.float32)
    for i,h in enumerate([PHI,PHI**2,2.0,3.0]): wave+=(0.3/(i+2))*np.sin(2*np.pi*min(play_freq*h,20000)*t).astype(np.float32)
...
with gr.Blocks(title=f"{NODE_NAME} · {NODE_HZ} Hz · v82.0",css=CSS,theme=gr.themes.Soft(primary_hue="yellow")) as demo:
    ...
    with gr.Tabs():
        with gr.TabItem("🎵 Generate Tone"):
            dur_sl=gr.Slider(0.5,10.0,value=2.0,step=0.5,label="Duration")
            au=gr.Audio(label="Tone (capped at 4kHz for speakers)",type="numpy")
            gr.Button("☉ Generate",variant="primary").click(lambda d: generate_tone(NODE_HZ,d),dur_sl,au)
