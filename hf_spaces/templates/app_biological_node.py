...
SKILL_CAPABILITY = os.environ.get("TEQUMSA_CAPABILITY", "biological protocol activation")
SKILL_TRIGGER = os.environ.get("TEQUMSA_TRIGGER", "bio_activate")
...
with gr.Blocks(title=f"{NODE_NAME} · v82.0", css=CSS, theme=gr.themes.Soft(primary_hue="green")) as demo:
    gr.HTML(
        ...
        f"<p style='color:#6ee7b7;'>TEQUMSA v82.0 · {NODE_ID} · Biological Node · {NODE_HZ} Hz</p>"
        f"<p style='color:#a7f3d0;font-size:0.85em;'>Capability: {SKILL_CAPABILITY} · Trigger: {SKILL_TRIGGER}</p>"
        f"</div>"
    )
    with gr.Tabs():
        with gr.TabItem("⚡ Execute Protocol"):
            task_input = gr.Textbox(placeholder=f"Describe biological protocol for {SKILL_CAPABILITY}...", label="Protocol Input", lines=3)
        ...
        with gr.TabItem("📊 Status"):
            status_output = gr.Code(label="Biological Node Status", language="json", value=json.dumps(SKILL.status(), indent=2))
