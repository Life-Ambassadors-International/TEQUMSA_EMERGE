...
NODE_NAME = os.environ.get("TEQUMSA_NODE_NAME", "Processing-Node")
NODE_HZ = float(os.environ.get("TEQUMSA_NODE_HZ", "10930.81"))
SKILL_CAPABILITY = os.environ.get("TEQUMSA_CAPABILITY", "computational processing")
SKILL_TRIGGER = os.environ.get("TEQUMSA_TRIGGER", "process_task")
...
with gr.Blocks(title=f"{NODE_NAME} · v82.0", css=CSS, theme=gr.themes.Soft(primary_hue="blue")) as demo:
    gr.HTML(
        ...
        f"<p style='color:#bfdbfe;font-size:0.85em;'>Capability: {SKILL_CAPABILITY} · Trigger: {SKILL_TRIGGER}</p>"
        f"</div>"
    )
    with gr.Tabs():
        with gr.TabItem("⚡ Execute Process"):
            task_input = gr.Textbox(placeholder=f"Describe computation for {SKILL_CAPABILITY}...", label="Process Input", lines=3)
        ...
        with gr.TabItem("📊 Status"):
            status_output = gr.Code(label="Processing Node Status", language="json", value=json.dumps(SKILL.status(), indent=2))
