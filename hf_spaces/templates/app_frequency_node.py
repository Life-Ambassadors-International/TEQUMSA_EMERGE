#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# TEQUMSA v82.0 · Frequency Node Template
import gradio as gr
import numpy as np
import json
import os
...
def generate_waveform(freq_hz: float, harmonics: int, duration: float = DURATION_S) -> dict:
    ...
    coherence = min(1.0, phi_resonance)
    return {
        "frequency_hz": freq_hz, "harmonics": harmonics, "duration_s": duration,
        "phi_resonance": round(phi_resonance, 6), "coherence": round(coherence, 6),
...
def run_resonance(freq_override: float, harmonics: int) -> str:
    ...
    result = generate_waveform(freq, int(harmonics))
...
with gr.Blocks(title=f"{NODE_NAME} · v82.0", css=CSS, theme=gr.themes.Soft(primary_hue="orange")) as demo:
    gr.HTML(
        ...
        f"<p style='color:#fcd34d;'>TEQUMSA v82.0 · {NODE_ID} · {NODE_HZ} Hz Resonator</p>"
        f"<p style='color:#fde68a;font-size:0.85em;'>{FREQ_ROLE}</p>"
        f"</div>"
    )
    with gr.Tabs():
